...
label_def:
    IDENTIFIER ':'                      { insert_symbol($1.name, $1.addr, 0); free($1.name); }
;
...
instruction:
  ...
  | JSR  IDENTIFIER                                 { emit_jb_format_identifier($1, 0b10101, $2.name); }
  | JSR  IMM5                                       { emit_jb_format_imm($1, 0b10101, $2); }
  | JSR  IMM8                                       { emit_jb_format_imm($1, 0b10101, $2); }
  | JSR  OFFSET                                     { emit_jb_format_imm($1, 0b10101, $2); }
  | RET                                             { emit_jb_format_imm($1, 0b10110, 0); }
  
  | HLT                                             { emit_special_format($1, 0b11000, 0, 0, 0); }
  | NOP                                             { emit_special_format($1, 0b11001, 0, 0, 0); }
  | NOP IDENTIFIER                                  { 
                                                              sort_symbols();
